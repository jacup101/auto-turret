...
async def run_face_detecion():
    ...
    # define serial port
    try:
        ser = serial.Serial('/dev/cu.usbmodem1402', 9600, serial.EIGHTBITS, serial.PARITY_NONE, serial.STOPBITS_ONE)
    except:
    ...
    while(True):
        ...
        await handle_movement(dir, speed, ser, use_serial)
...
async def handle_movement(dir, speed, ser, use_serial):
    right_char = 'r'.encode("ascii")
    new_line = '\n'.encode("ascii")
    shoot_char = 's'.encode("ascii")
    left_char = 'l'.encode("ascii")
    ...
    if "right" in dir:
        ...
        if use_serial:
            await send_communications(right_char, ser)
    if "shoot" in dir:
        # do whatever we need to do here
        print("shoot")
        if use_serial:
            await send_communications(shoot_char, ser)
    if "left" in dir:
        # do whatever we need to do here
        print("move left by " + str(speed))
        if use_serial:
            await send_communications(left_char, ser)
